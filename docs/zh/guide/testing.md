...
在可控的服务 fixture 上验证请求映射、媒体传输与任务推进。[项目 Provider 示例](https://github.com/hypit-ai/hypit/tree/main/examples/provider-package)
...
不得提交客户或品牌 fixture、凭据痕迹、付费产物、工作站绝对路径与一次性交付脚本。通用的真实调用测试仅可在显式 opt-in、不提交密钥、且未设置开关时会在花钱之前失败的前提下保留。本机 ffmpeg、浏览器或 OpenCV 的集成测试同样需要显式 opt-in，且不进入默认测试成功的必要条件。
